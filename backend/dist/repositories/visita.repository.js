...
exports.VisitaRepository = {
    async findVisitasByFecha(fecha) {
        const query = `
        SELECT 
            v.*,
    ...
    `;
        const res = await db_1.pool.query(query, [fecha]);
        return res.rows;
    },
    async findHistorialCompleto(page = 1, pageSize = 50) {
      ...
      LIMIT $1 OFFSET $2;
    `;
        const result = await db_1.pool.query(query, [pageSize, offset]);
        return result.rows;
    },
    async getAgrupadoPorMes(anio, mes) {
        const query = `
      SELECT 
      ...
      GROUP BY fecha_str;
    `;
        const result = await db_1.pool.query(query, [anio, mes]);
        return result.rows;
    },
    async getById(id) {
        const query = `
        ...
        WHERE v.id = $1
    `;
        const res = await db_1.pool.query(query, [id]);
        return res.rows[0];
    },
    async updateVisita(id, datos) {
        const query = `
      UPDATE Visita 
      SET 
