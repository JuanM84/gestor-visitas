...
exports.EstadisticasService = {
    async getDashboardAdminStats(mes, anio) {
        const kpisQuery = `
            SELECT 
        ...
        `;
        const kpisResult = await db_1.pool.query(kpisQuery, [mes, anio]);
        ...
        const rankingQuery = `
            ...
            LIMIT 5
        `;
        const rankingResult = await db_1.pool.query(rankingQuery, [mes, anio]);
        const evolucionQuery = `
            ...
            ORDER BY dia ASC
        `;
        const evolucionResult = await db_1.pool.query(evolucionQuery, [mes, anio]);
        ...
        const distribucionTipoQuery = `
            ...
            ORDER BY total_personas DESC
        `;
        const distribucionTipoResult = await db_1.pool.query(distribucionTipoQuery, [mes, anio]);
        ...
        const desglosePorNivelQuery = `
            ...
            ORDER BY total_personas DESC
        `;
        const desglosePorNivelResult = await db_1.pool.query(desglosePorNivelQuery, [mes, anio]);
        return {
            kpis: {
    ...
    async getStatsByRango(fechaDesde, fechaHasta) {
        const kpisQuery = `
            SELECT
                COUNT(*) as total_visitas,
                COALESCE(SUM(cantidad_personas), 0) as total_personas,
        ...
        `;
        const kpisResult = await db_1.pool.query(kpisQuery, [fechaDesde, fechaHasta]);
        const kpis = kpisResult.rows[0];
        const rankingQuery = `
            SELECT g.nombre, SUM(v.cantidad_personas) as total_personas, COUNT(v.id) as cantidad_visitas
            FROM Visita v
            ...
            LIMIT 5
        `;
        const rankingResult = await db_1.pool.query(rankingQuery, [fechaDesde, fechaHasta]);
        const evolucionQuery = `
            ...
            ORDER BY fecha ASC
        `;
        const evolucionResult = await db_1.pool.query(evolucionQuery, [fechaDesde, fechaHasta]);
        const distribucionTipoQuery = `
            SELECT
                gr.tipo_visitante,
                COUNT(v.id)              AS cantidad_visitas,
            ...
            ORDER BY total_personas DESC
        `;
        const distribucionTipoResult = await db_1.pool.query(distribucionTipoQuery, [fechaDesde, fechaHasta]);
        const desglosePorNivelQuery = `
            SELECT
                gr.nivel_educativo,
                COUNT(v.id)              AS cantidad_visitas,
            ...
            ORDER BY total_personas DESC
        `;
        const desglosePorNivelResult = await db_1.pool.query(desglosePorNivelQuery, [fechaDesde, fechaHasta]);
        return {
            kpis: {
                visitas: parseInt(kpis.total_visitas),
                personas: parseInt(kpis.total_personas),
